...
def conv_layer(inputs, kernel_size, stride, num_features, idx, linear = False):
  with tf.variable_scope('{0}_conv'.format(idx), reuse = tf.AUTO_REUSE) as scope:
    ...
    if linear:
      return conv_biased
    conv_rect = tf.nn.leaky_relu(conv_biased,name='{0}_conv'.format(idx), alpha = 0.2)
    return conv_rect

def transpose_conv_layer(inputs, kernel_size, stride, num_features, idx, linear = False):
  with tf.variable_scope('{0}_trans_conv'.format(idx), reuse = tf.AUTO_REUSE) as scope:
    input_channels = inputs.get_shape()[3]
    ...
    if linear:
      return conv_biased
    conv_rect = tf.nn.leaky_relu(conv_biased,name='{0}_transpose_conv'.format(idx), alpha = 0.2)
    return conv_rect
...
def fc_layer(inputs, hiddens, idx, flat = False, linear = False):
  with tf.variable_scope('{0}_fc'.format(idx)) as scope:
    ...
    return tf.nn.sigmoid(ip,name=str(idx)+'_fc')
